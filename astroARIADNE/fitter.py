...
try:
    ...
    warnings.warn("(py)MultiNest installation (or libmultinest.dylib) not detected.")
...
class Fitter:
    ...
    def initialize(self):
        ...
        if self._bma:
            ...
            for mod in self._bma_models:
                ...
                if (
                    (mod.lower() in ["btcond", "btnextgen"] and self.star.temp > 4000)
                    or (mod.lower() in ["ck04", "kurucz"] and self.star.temp < 4000)
                    or (mod.lower() == "coelho" and self.star.temp < 3500)
                ):
                    continue
    ...
    def _default_priors(self):
        ...
        if self.star.get_logg:
            defaults["logg"] = st.norm(loc=self.star.logg, scale=self.star.logg_e)
        else:
    ...
    def fit_bma(self):
        ...
        avgd = self.bayesian_model_average(
            outs, self._grids, self._norm, self.n_samples, c
        )
        self.save_bma(avgd)
    ...
    def save(self, out_file, results=None):
        ...
        for i in range(posterior_samples.shape[0]):
            ...
            out["posterior_samples"]["loglike"][i] = log_likelihood(
                theta, flux, flux_er, wave, filts, interpolator, self.norm, av_law
            )
        ...
        if not self.bma:
            ...
            for i, param in enumerate(order):
                if not self.coordinator[i]:
                    ...
                    if param == "z":
                        ...
                        logdat = out_filler(
                            samp, logdat, param, "(R/D)^2", out, fmt="e"
                        )
                        if star.dist != 1:
                            logdat = out_filler(
                                out["posterior_samples"]["rad"], logdat, "rad", "R", out
                            )
                    else:
            ...
            out["best_fit"]["loglike"] = log_likelihood(
                best_theta, flux, flux_er, wave, filts, interpolator, self.norm, av_law
            )
    ...
    def save_bma(self, avgd):
            ...
            if not self.coordinator[i]:
                ...
                if param == "z":
                    logdat_samples = out_filler(
                        samp, logdat_samples, param, "[Fe/H]", out, method="samples"
                    )
                    logdat_average = out_filler(
                        sampw, logdat_average, param, "[Fe/H]", out, method="averaged"
                    )
                elif param == "norm":
                    logdat_samples = out_filler(
                        samp,
                    ...
                    logdat_samples = out_filler(
                        samp, logdat_samples, param, param, out, method="samples"
                    )
                    logdat_average = out_filler(
                        sampw, logdat_average, param, param, out, method="averaged"
                    )
            else:
                logdat_samples = out_filler(
                    0,
        ...
        logdat_samples = out_filler(
            samp, logdat_samples, "grav_mass", "grav_mass", out, method="samples"
        )
        logdat_average = out_filler(
            sampw, logdat_average, "grav_mass", "grav_mass", out, method="averaged"
        )
        ...
        logdat_samples = out_filler(
            samp, logdat_samples, "lum", "lum", out, method="samples"
        )
        logdat_average = out_filler(
            sampw, logdat_average, "lum", "lum", out, method="averaged"
        )
        ...
        if not use_norm:
            ...
            logdat_samples = out_filler(
                samp, logdat_samples, "AD", "AD", out, method="samples"
            )
            logdat_average = out_filler(
                sampw, logdat_average, "AD", "AD", out, method="averaged"
            )
        ...
        age_samp, mass_samp, eep_samp = self.estimate_age(
            out["best_fit_samples"], out["uncertainties_samples"], c=choice(self.colors)
        )
        ...
        logdat_samples = out_filler(
            age_samp, logdat_samples, "age", "age", out, method="samples"
        )
        logdat_samples = out_filler(
            mass_samp, logdat_samples, "iso_mass", "iso_mass", out, method="samples"
        )
        logdat_samples = out_filler(
            eep_samp, logdat_samples, "eep", "eep", out, method="samples"
        )
        ...
        for i, param in enumerate(order):
            if not self.coordinator[i]:
                ...
                sampw = out["weighted_average"][param]
                logdat_samples = out_filler(
                    samp, logdat_samples, param, param, out, fmt="f", method="samples"
                )
                logdat_average = out_filler(
                    sampw, logdat_average, param, param, out, fmt="f", method="averaged"
                )
        ...
        # Load Mamajek spt table
        mamajek_spt, mamajek_temp = np.loadtxt(
            f"{filesdir}/mamajek_spt.dat", dtype=str, usecols=[0, 1], unpack=True
        )
    ...
    def bayesian_model_average(outputs, grids, norm, nsamples, c="white"):
        ...
        for k in tqdm(post_samples[0].keys()):
            ...
            avg_kde = st.gaussian_kde(
                np.concatenate(traces), weights=np.concatenate(extended_weights)
            )
    ...
    def _get_rad(samples, dist, dist_e):
        ...
        d = st.truncnorm(loc=dist, scale=dist_e, a=a, b=b).rvs(size=norm.shape[0])
        n = np.sqrt(norm)
        r = n * d  # This is in pc
    ...
    def estimate_age(self, bf, unc, c="white"):
        ...
        """
        print(colored("\t\t*** ESTIMATING AGE AND MASS USING MIST ISOCHRONES ***", c))
        ...
        if self.star.lum != 0 and self.star.lum_e != 0:
            params["logL"] = (np.log10(bf["lum"]), abs(np.log10(max(unc["lum"]))))
...
def dynesty_log_like(
    cube, flux, flux_er, filts, coordinator, fixed, use_norm, wave, interpolator, av_law
):
    """Dynesty log likelihood wrapper."""
    theta = build_params(cube, flux, flux_er, filts, coordinator, fixed, use_norm)
    return log_likelihood(
        theta, flux, flux_er, wave, filts, interpolator, use_norm, av_law
    )


def pt_dynesty(cube, flux, flux_er, filts, prior_dict, coordinator, use_norm):
    """Dynesty prior transform."""
    return prior_transform_dynesty(
        cube, flux, flux_er, filts, prior_dict, coordinator, use_norm
    )
...
def multinest_log_like(cube, ndim, nparams):
    ...
    theta = build_params(theta, flux, flux_er, filts, coordinator, fixed, use_norm)
    return log_likelihood(
        theta, flux, flux_er, wave, filts, interpolator, use_norm, av_law
    )
