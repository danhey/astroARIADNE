...
class SEDPlotter:
        ...
        if input_files != "raw":
            ...
            if model is None:
                if self.engine != "Bayesian Model Averaging":
                    ...
                    self.bma = True
                    zs = np.array([out["lnZ"][key] for key in out["lnZ"].keys()])
            ...
            if not self.norm:
                ...
                self.order = np.array(["teff", "logg", "z", "norm", "Av"])
            ...
            model_samples = model_grid(
                theta_samples, filters, wave, self.interpolator, self.norm, self.av_law
            )
            model_average = model_grid(
                theta_average, filters, wave, self.interpolator, self.norm, self.av_law
            )
            if self.irx:
                irx_model_sam = model_grid(
                    theta_samples,
                    irx_filt,
                    irx_wave,
                    self.interpolator,
    ...
    def plot_SED_no_model(self, s=None):
        ...
        ax.set_ylabel(
            r"$\lambda$F$_\lambda$ (erg cm$^{-2}$s$^{-1}$)",
        ...
        ax.tick_params(axis="both", which="minor", labelsize=self.tick_labelsize)
        ...
        if self.pdf:
            plt.savefig(self.out_folder + "/SED_no_model.pdf", bbox_inches="tight")
        if self.png:
            plt.savefig(self.out_folder + "/SED_no_model.png", bbox_inches="tight")
        pass
    ...
    def plot_SED(self):
        ...
        if self.irx:
            ax.errorbar(
                self.irx_wave,
                self.irx_flux * self.irx_wave,
                xerr=self.irx_bandpass,
                yerr=self.irx_flux_er,
                fmt=",",
                ecolor=self.irx_error_color,
                zorder=0,
                marker=None,
            )
            ax.scatter(
        ...
        # Residual plot
        ax_r.axhline(y=0, lw=2, ls="--", c="k", alpha=0.7)
        ...
        ax_r.errorbar(
            ...
            zorder=3,
            xerr=self.bandpass,
            yerr=np.array(self.flux_er) / errors,
            fmt=",",
            ecolor=self.error_color,
            marker=None,
        )
        ...
        if self.irx:
            ...
            ax_r.errorbar(
                self.irx_wave,
                np.zeros(self.irx_wave.shape[0]),
                xerr=self.irx_bandpass,
                yerr=self.irx_flux_er,
                fmt=",",
                ecolor=self.irx_error_color,
        ...
        ax_r.set_xlabel(
            r"$\lambda (\mu m)$", fontsize=self.fontsize, fontname=self.fontname
        )
        ax.set_ylabel(
            r"$\lambda$F$_\lambda$ (erg cm$^{-2}$s$^{-1}$)",
            fontsize=self.fontsize,
        ...
        ax_r.set_ylabel(
            "Residuals\n$(\\sigma)$", fontsize=self.fontsize, fontname=self.fontname
        )

        ax.tick_params(axis="both", which="major", labelsize=self.tick_labelsize)
        ax.tick_params(axis="both", which="minor", labelsize=self.tick_labelsize)
        ax_r.tick_params(axis="both", which="major", labelsize=self.tick_labelsize)
        ...
        if (
            "GALEX_FUV" in self.star.filter_names[self.star.filter_mask]
            or "GALEX_NUV" in self.star.filter_names[self.star.filter_mask]
        ):
            ax.set_xlim(xlims1)
        ...
        if self.save:
            ...
            np.savetxt(
                f"{self.out_folder}/synthetic.dat",
                data,
                fmt="%s",
    ...
    def SED(self, ax):
        ...
        if self.grid == "phoenix":
            ...
            brf, _ = pyasl.instrBroadGaussFast(
                new_ww, flux, 1500, edgeHandling="firstlast", fullout=True, maxsig=8
            )
            brf = brf[lower_lim * upper_lim]
            brf = apply(ext, brf)
            flx = brf * norm * new_w
            ax.plot(
                new_w[:-1000], flx[:-1000], lw=1.25, color=self.model_color, zorder=0
            )
            ...
            brf, _ = pyasl.instrBroadGaussFast(
                new_w, flux, 1500, edgeHandling="firstlast", fullout=True, maxsig=8
            )
            flx = apply(ext, brf)
            flx *= wave * norm
    ...
    def plot_trace(self):
        ...
        for i, param in enumerate(self.order):
            if not self.coordinator[i]:
                ...
                ax.step(
                    range(len(samples[param])), samples[param], color="k", alpha=0.8
                )
                ax.set_ylabel(param, fontsize=self.fontsize, fontname=self.fontname)
                ax.set_xlabel("Steps", fontsize=self.fontsize, fontname=self.fontname)
                ...
                plt.savefig(self.chain_out + "/" + param + ".png", bbox_inches="tight")
        plt.close("all")
        pass
    ...
    def plot_like(self):
        ...
        for i, param in enumerate(self.order):
            if not self.coordinator[i]:
                ...
                ax.set_ylabel(
                    "log likelihood", fontsize=self.fontsize, fontname=self.fontname
                )
                ax.set_xlabel(param, fontsize=self.fontsize, fontname=self.fontname)
                ax.tick_params(
                    axis="both", which="major", labelsize=self.tick_labelsize
                )
                plt.savefig(self.like_out + "/" + param + ".png", bbox_inches="tight")
        plt.close("all")
    ...
    def plot_post(self):
        ...
        for i, param in enumerate(self.order):
            if not self.coordinator[i]:
                f, ax = plt.subplots(figsize=(12, 4))
                ax.scatter(samples[param], samples["posteriors"], alpha=0.5, s=40)
                best = self.out["best_fit"][param]
                # ax.axvline(np.median(samples[param]), color='red', lw=1.5)
                ax.axvline(best, color="red", lw=1.5)
                ax.set_ylabel(
                    "log posterior", fontsize=self.fontsize, fontname=self.fontname
                )
                ax.set_xlabel(param, fontsize=self.fontsize, fontname=self.fontname)
                ax.tick_params(
                    axis="both", which="major", labelsize=self.tick_labelsize
                )
                plt.savefig(self.post_out + "/" + param + ".png", bbox_inches="tight")
        plt.close("all")
        pass
    ...
    def plot_bma_hist(self):
        ...
        for i, param in enumerate(self.order):
            ...
            if not self.coordinator[i]:
                ...
                for j, m in enumerate(models):
                    ...
                    ax2.plot(xx1, kde(xx2) * popt[2], lw=2, alpha=1, color=colors[j])
                # The same but for the weighted samples
                n, bins, patches = ax1.hist(
                    self.out["weighted_samples"][param],
                    alpha=0.3,
                    bins=20,
                ...
                n, bins, patches = ax1.hist(
                    self.out["weighted_average"][param],
                    alpha=0.3,
                    bins=20,
                ...
                if param == "teff":
                    ...
                    lab = r"Mass (M$_\odot$)"
                # Normal
                ax1.set_ylabel("PDF", fontsize=self.fontsize, fontname=self.fontname)
                # Weighted
                ax2.set_ylabel("N", fontsize=self.fontsize, fontname=self.fontname)
                axes = [ax1, ax2]
                for ax in axes:
                    ax.set_xlabel(lab, fontsize=self.fontsize, fontname=self.fontname)
                    for tick in ax.get_yticklabels():
                        tick.set_fontname(self.fontname)
                ...
                if self.png:
                    f1.savefig(
                        self.hist_out + "/" + param + ".png", bbox_inches="tight"
                    )
                    f2.savefig(
                        self.hist_out + "/weighted_" + param + ".png",
                        bbox_inches="tight",
                    )
                if self.pdf:
                    f1.savefig(
                        self.hist_out + "/" + param + ".pdf", bbox_inches="tight"
                    )
                    f2.savefig(
                        self.hist_out + "/weighted_" + param + ".pdf",
                        bbox_inches="tight",
                    )
                plt.close(f1)
                plt.close(f2)
        ...
        if self.bma:
            ...
            n, bins, patches = ax.hist(
                samp, alpha=0.3, bins=20, label="MIST", density=True
            )
            kde = gaussian_kde(samp)
            xx = np.linspace(bins[0], bins[-1], 10000)
            ax.plot(xx, kde(xx), color="k", lw=2, alpha=0.7)
            ax.set_ylabel("PDF", fontsize=self.fontsize, fontname=self.fontname)
            ax.set_xlabel("Age", fontsize=self.fontsize, fontname=self.fontname)
            ...
            plt.legend(loc=0)
            if self.png:
                plt.savefig(self.hist_out + "/age.png", bbox_inches="tight")
            if self.pdf:
                plt.savefig(self.hist_out + "/age.pdf", bbox_inches="tight")
            plt.close(f)
            # Mass hist
            f, ax = plt.subplots(figsize=(12, 4))
            ...
            ax.plot(xx, kde(xx), color="k", lw=2, alpha=0.7)
            ax.set_ylabel("PDF", fontsize=self.fontsize, fontname=self.fontname)
            ax.set_xlabel("Mass", fontsize=self.fontsize, fontname=self.fontname)
            ...
            plt.legend(loc=0)
            if self.png:
                plt.savefig(self.hist_out + "/iso_mass.png", bbox_inches="tight")
            if self.pdf:
                plt.savefig(self.hist_out + "/iso_mass.pdf", bbox_inches="tight")
            plt.close(f)
            # EEP hist
            f, ax = plt.subplots(figsize=(12, 4))
            ...
            ax.plot(xx, kde(xx), color="k", lw=2, alpha=0.7)
            ax.set_ylabel("PDF", fontsize=self.fontsize, fontname=self.fontname)
            ax.set_xlabel("EEP", fontsize=self.fontsize, fontname=self.fontname)
            ...
            plt.legend(loc=0)
            if self.png:
                plt.savefig(self.hist_out + "/EEP.png", bbox_inches="tight")
            if self.pdf:
                plt.savefig(self.hist_out + "/EEP.pdf", bbox_inches="tight")
            plt.close(f)
    ...
    def plot_bma_HR(self, nsamp):
        ...
        lc = LineCollection(segments, cmap=self.hr_cmap, norm=norm, linewidths=5)
        ...
        ax.scatter(
            ...
            zorder=1002,
            edgecolors="k",
            marker=self.hr_marker,
        )

        ax.invert_xaxis()
        ax.set_xlabel("logTeff", fontsize=self.fontsize, fontname=self.fontname)
        ax.set_ylabel("logL", fontsize=self.fontsize, fontname=self.fontname)
        ax.tick_params(axis="both", which="major", labelsize=self.tick_labelsize)
        ...
        if self.png:
            plt.savefig(self.out_folder + "/HR_diagram.png", bbox_inches="tight")
        if self.pdf:
            plt.savefig(self.out_folder + "/HR_diagram.pdf", bbox_inches="tight")
    ...
    def plot_corner(self):
        ...
        for i in range(theta.shape[0]):
            ax = axes[i, i]
            ax.axvline(
                theta[i], color=self.corner_med_c, linestyle=self.corner_med_style
            )
            ax.axvline(
                theta_lo[i], color=self.corner_v_c, linestyle=self.corner_v_style
            )
            ax.axvline(
                theta_up[i], color=self.corner_v_c, linestyle=self.corner_v_style
            )
            t = titles[i]

            ax.set_title(t, fontsize=self.corner_fontsize, fontname=self.fontname)
        ...
        for yi in range(theta.shape[0]):
            for xi in range(yi):
                ...
                ax.axvline(
                    theta[xi], color=self.corner_med_c, linestyle=self.corner_med_style
                )
                ax.axhline(
                    theta[yi], color=self.corner_med_c, linestyle=self.corner_med_style
                )
            ...
            if self.pdf:
                plt.savefig(f"{self.out_folder}/CORNER.pdf", bbox_inches="tight")
            if self.png:
                plt.savefig(f"{self.out_folder}/CORNER.png", bbox_inches="tight")
        pass
    ...
    def fetch_Phoenix(self):
        ...
        selected_SED += "/lte"
        selected_SED += (
            str(sel_teff) if len(str(sel_teff)) == 5 else "0" + str(sel_teff)
        )
    ...
    def fetch_btsettl(self):
        ...
        selected_SED += (
            str(sel_teff) if len(str(sel_teff)) == 3 else "0" + str(sel_teff)
        )
    ...
    def __create_titles(self, titles, theta, theta_up, theta_lo):
        ...
        for i, param in enumerate(titles):
            ...
            if param == "rad" or param == "dist":
                ...
                new_titles[i] += r"$^{+" + "{:.3f}".format(theta_up[i] - theta[i])
                new_titles[i] += r"}_{-" + "{:.3f}".format(theta[i] - theta_lo[i])
                new_titles[i] += r"}$"
            else:
